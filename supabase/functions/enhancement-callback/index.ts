...
serve(async (req) => {
  ...
  try {
    ...
    // We need to get the original product data from the productId
    // The productId format is like "temp-1234567890-0"
    // We need to extract the original product info from localStorage or find another way
    
    // For now, let's create products with the data we have
    ...
    for (let i = 0; i < generatedImages.length; i++) {
      try {
        ...
        // Generate caption for this version
        ...
        const { data: captionData, error: captionError } = await supabase.functions.invoke('generate-caption', {
          body: {
            productName: `Enhanced Product Version ${i + 1}`,
            price: null,
            description: generatedImage.prompt, // Use the enhancement prompt as description
            contentType: 'product',
        ...
        if (!originalProductData.user_id) {
          ...
          // Skip this product creation
          continue;
        }
        
        // Create new product record with enhanced image
        const newProductData = {
          ...
          caption: captionData?.caption || `Generated caption ${i + 1}`,
          is_new: true,
