...
import { MessageSquare, Plus, Calendar as CalendarIcon, ArrowRight, Package, Camera } from 'lucide-react';
...
const QuickActions = ({ onCaptionsClick, onScheduleClick }: QuickActionsProps) => {
  ...
  const handleStartAddProducts = () => {
    ...
    // Show content type selection dialog
  ...
  const handleGeneralContent = () => {
    setShowContentTypeDialog(false);
    navigate('/general-content-upload');
  };
    ...
    <>
      ...
      <Dialog open={showContentTypeDialog} onOpenChange={setShowContentTypeDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-center">{t('contentType.title')}</DialogTitle>
          ...
          <div className="grid gap-4 py-4">
            <Button 
              variant="outline" 
              className="w-full h-16 flex flex-col gap-2 items-center justify-center border-2 hover:bg-blue-50 hover:border-blue-300 dark:hover:bg-blue-900/20" 
              onClick={handleProductContent}
            >
              <Package className="h-6 w-6 text-blue-600" />
              <span className="text-sm text-center leading-tight">{t('contentType.productContent')}</span>
            </Button>
            <Button 
              variant="outline" 
              className="w-full h-16 flex flex-col gap-2 items-center justify-center border-2 hover:bg-green-50 hover:border-green-300 dark:hover:bg-green-900/20" 
              onClick={handleGeneralContent}
            >
              <Camera className="h-6 w-6 text-green-600" />
              <span className="text-sm text-center leading-tight">{t('contentType.generalContent')}</span>
            </Button>
          </div>
        </DialogContent>
      </Dialog>
